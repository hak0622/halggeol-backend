...
public class AggressiveInsightStrategy implements InsightDetailStrategy {
    private final InsightDetailMapper mapper;

    @Override
    public ProductType getProductType() {
        return ProductType.AGGRESSIVE;
    }

    @Override
    public InsightDetailResponseDTO fetchProductInfo(int round, String productId, int userId) {
        return mapper.getAggressivePensionInfo(round, productId, userId);
    }
    ...
    public List<ProfitSimulationDTO> fetchSimulation(int round, String productId, int userId, LocalDate recDate, LocalDate anlzDate) {
        return mapper.getProfitSimulation(round, productId, userId, "aggressive", recDate, anlzDate);
    }
}
